...
class Perhitungan extends CI_Controller
{
	...
	{
		echo '<b>Proses Data Penilaian dalam Penentuan Kelayakan Penerimaan Bantuan</b></br></br>';
		echo '<b>Langkah-Langkah Metode PSI (<i>Preference Selection Index</i>) :</b></br>';

		echo '<b>1. Membuat Matriks Keputusan </b></br>';
		...
		$step2_4 = 0;
		$step2_5 = 0;
		echo '<b>2. Melakukan Normalisasi Matriks </b></br>';
		for ($i = 1; $i <= $jumlahData; $i++) :
			// Benefit
		...
		echo '</br>';
		echo '<b>3. Mencari nilai rata-rata matriks (Jumlah Data : ' . $jumlahData . ' data)</b></br>';
		echo round($step2_1, 4) . ' | ';
		echo round($step2_2, 4) . ' | ';
		echo round($step2_3, 4) . ' | ';
		echo round($step2_4, 4) . ' | ';
		echo round($step2_5, 4) . '</br></br>';

		// 3. Rata-Rata Matriks
		echo '<b>Hasil nilai rata-rata : </b></br>';
		...
		echo "<b>4.	Menentukan Nilai Variasi Preferensi</b></br>";
		$x_1 = 0;
		$x_2 = 0;
		$x_3 = 0;
		$x_4 = 0;
		...
		echo '<b>5.	Menentukan Penyimpangan Nilai Preferensi</b></br>';
		$dev_1 = 1 - $x_1;
		...
		// 6. Bobot Kriteria
		echo '<b>6. Menentukan Bobot kriteria</b></br>';
		...
		// 7. Indeks Preferensi
		echo '<b>7. Menghitung Pemilihan Index Preferensi</b></br>';
		$sum_index = 0;
		for ($i = 1; $i <= $jumlahData; $i++) :
		...
		echo '<b>Total Indeks Preferensi : </b>' . str_pad(round($sum_index, 4), 6, '0') . '</br></br>';

		//8. Menentukan Status Kelayakan
		echo '<b>8. Menentukan Status Kelayakan</b></br></br>';
		echo '<b>Batas Prioritas (Total Indeks Preferensi / Jumlah data : ' . $jumlahData . ' data)</b></br>';
		$prior = $sum_index / $jumlahData;
		echo str_pad(round($prior, 4), 6, '0') . '</br></br>';

		echo '<b>Status kelayakan</b></br>';
		for ($i = 1; $i <= $jumlahData; $i++) :
			if ($data['total_i_' . $i] >= $prior) {
				$data['status_' . $i] = 'Layak';
				echo ${'kode' . $i} . ' = ' . $data['status_' . $i] . '<br/>';
			} else {
